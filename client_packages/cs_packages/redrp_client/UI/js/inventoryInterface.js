...
$(document).ready(function () {
    mp.trigger('requestInventoryData');
});
...
function inventoryInitialization(leftTitle, leftCapacity, leftCapacityMax, leftItems, leftLocked, rightTitle, rightCapacity, rightCapacityMax, rightItems) {
    ...
    $('.itemLeft').click(function () {
        ...
        if (typeof hasTitle !== typeof undefined && hasTitle !== false) {
            ...
            doubleClick++;
            if (doubleClick === 2) {
                mp.trigger('sendItemDoubleClick', false, itemId);
                doubleClick = 0;
            } else {
                setTimeout(function () {
                    if (doubleClick === 1) {
                        mp.trigger('sendItemClick', false, itemId);
                        doubleClick = 0;
                    }
                }, 300);
            }
    ...
    $('.itemLeft').contextmenu(function (e) {
        ...
        if (typeof hasTitle !== typeof undefined && hasTitle !== false) {
            ...
            var startItem = pageLeft * 30 - 30;
            itemId = itemId + startItem;
            mp.trigger('sendItemRightClick', false, itemId);
        }
    });

    //Handles the left grid return button click event
    $('#returnLeft').click(function () {
        mp.trigger('sendReturnClick', false);
    });
    ...
    $('.itemRight').click(function () {
        ...
        if (typeof hasTitle !== typeof undefined && hasTitle !== false) {
            ...
            doubleClick++;
            if (doubleClick === 2) {
                mp.trigger('sendItemDoubleClick', true, itemId);
                doubleClick = 0;
            } else {
                setTimeout(function () {
                    if (doubleClick === 1) {
                        mp.trigger('sendItemClick', true, itemId);
                        doubleClick = 0;
                    }
                }, 300);
            }
